...
public class DocumentService {
    ...
    public ImageFileResponse fetchImages(final FileDto fileDto, final Integer projectId) {
        ...
        //서버에 파일 저장
        final List<ImageDto> imageDtos = documentImageService.saveDocumentImage(documentImageDtos, fileDto.getProjectFolderId(),
                execute.getFileDtos(), projectId);

        //무조건 데이터가 1개 이상 있음, response 구성
        final List<ImageData> imageData = relateDocumentImages(execute.getFileDtos(), imageDtos.get(0).docImageId(),
                imageDtos);
    ...
    // 이미지 데이터와 이미지 파일을 매핑
    private List<ImageData> relateDocumentImages(final List<ImageFileDto> imageFileDtos, final Integer docId, final List<ImageDto> imageDtos) {
    ...
    public void saveDocument(final Document document, final Integer projectId) {
        ...
        fileService.createFolder(
                Folder.DOCUMENT.getDocumentName(projectId, save.getProjectFolder().getProjectFolderId(), save.getDocId()));
